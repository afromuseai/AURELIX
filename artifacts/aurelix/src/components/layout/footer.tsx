...
const socialLinks = [
  { name: 'Twitter', icon: Twitter, href: 'https://twitter.com/aurelixsystems' },
  { name: 'LinkedIn', icon: Linkedin, href: 'https://linkedin.com/company/aurelixsystems' },
  { name: 'GitHub', icon: Github, href: 'https://github.com/aurelixsystems' },
...
const footerLinks = [
  {
    ...
    links: [
      ...
      { name: 'STAGEONE', href: '/#products' },
      { name: 'Cyber Intelligence', href: '/#products' },
    ],
  },
  {
    title: 'Company',
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12 lg:gap-8">
          ...
          <div className="lg:col-span-2">
            <Link href="/" className="flex items-center gap-3 mb-6">
              ...
              <div>
                <span className="font-display text-xl tracking-wider text-gold-gradient block">AURELIX</span>
                <span className="text-xs tracking-[0.3em] text-muted-foreground">SYSTEMS</span>
              </div>
            </Link>
          ...
          {footerLinks.map((column) => (
            <div key={column.title}>
              <h3 className="font-display text-sm tracking-wider text-foreground mb-4">{column.title}</h3>
              <ul className="space-y-3">
                {column.links.map((link) => (
                  <li key={link.name}>
                    <Link href={link.href} className="text-sm text-muted-foreground hover:text-gold transition-colors duration-300">
                      {link.name}
                    </Link>
                  </li>
                ))}
